...
Level::Level(const std::vector<std::string>& levelDescription)
{
	...
	m_playerRespawn_1 = { BLOCK_SIZE * (m_width / 2 + 3), BLOCK_SIZE / 2 };
	...
	unsigned int currentBottomOffset = static_cast<size_t>(BLOCK_SIZE) * (m_height - 1) + BLOCK_SIZE / 2.f;
	...
	m_mapObjects.emplace_back(
		std::make_shared<Border>(
			glm::vec2(BLOCK_SIZE, BLOCK_SIZE / 2.f + m_height * BLOCK_SIZE),
			glm::vec2(m_width * BLOCK_SIZE, BLOCK_SIZE / 2.f),
			0.f,
			0.f
		)
	...
	m_mapObjects.emplace_back(
		std::make_shared<Border>(
			glm::vec2(0, 0),
			glm::vec2(BLOCK_SIZE, (1 + m_height) * BLOCK_SIZE),
			0.f,
			0.f
		)
	);
	...
	m_mapObjects.emplace_back(
		std::make_shared<Border>(
			glm::vec2(BLOCK_SIZE + m_width * BLOCK_SIZE, 0),
			glm::vec2(BLOCK_SIZE * 2, (1 + m_height) * BLOCK_SIZE),
			0.f,
			0.f
		)
	);
