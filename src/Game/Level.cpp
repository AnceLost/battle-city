...
Level::Level(const std::vector<std::string>& levelDescription)
{
	...
	unsigned int currentBottomOffset = static_cast<size_t>(BLOCK_SIZE) * (m_heightBlocks - 1) + BLOCK_SIZE / 2.f;
		...
		for (const char currentElement : currentRow)
		{
			...
			default:
				m_mapObjects.emplace_back(
					createGameObjectFromDescription(
						currentElement,
	...
	m_mapObjects.emplace_back(
		std::make_shared<Border>(
			glm::vec2(BLOCK_SIZE, BLOCK_SIZE / 2.f + m_heightBlocks * BLOCK_SIZE),
			glm::vec2(m_widthBlocks * BLOCK_SIZE, BLOCK_SIZE / 2.f),
			0.f,
			0.f
		)
	...
	m_mapObjects.emplace_back(
		std::make_shared<Border>(
			glm::vec2(0, 0),
			glm::vec2(BLOCK_SIZE, (1 + m_heightBlocks) * BLOCK_SIZE),
			0.f,
			0.f
		)
	);
	...
	m_mapObjects.emplace_back(
		std::make_shared<Border>(
			glm::vec2(BLOCK_SIZE + m_widthBlocks * BLOCK_SIZE, 0),
			glm::vec2(BLOCK_SIZE * 2, (1 + m_heightBlocks) * BLOCK_SIZE),
			0.f,
			0.f
		)
	);
...
std::vector<std::shared_ptr<IGameObject>> Level::getObjectsInArea(
	...
	glm::vec2 bottomLeft_converted(
		std::clamp(bottomLeft.x - BLOCK_SIZE, 0.f, static_cast<float>(m_widthPixels)),
		std::clamp(m_heightPixels - bottomLeft.y + BLOCK_SIZE / 2, 0.f, static_cast<float>(m_heightPixels))
	);
	glm::vec2 topRight_converted(
		std::clamp(topRight.x - BLOCK_SIZE, 0.f, static_cast<float>(m_widthPixels)),
