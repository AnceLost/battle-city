...
std::shared_ptr<RenderEngine::Sprite> ResourceManager::loadSprite(
    ...
    if (!pShader)
    {
        std::cerr << "Can't find the shader: " << shaderName 
            << " for the sprite: " << spriteName << std::endl;
    }
...
std::shared_ptr<RenderEngine::Texture2D> ResourceManager::loatTextureAtlas(
    const std::string textureName,
...
bool ResourceManager::loadJSONResources(const std::string& JSONPath)
{
    ...
    if (!parseResult)
    {
        std::cerr << "JSON parse error (" << JSONPath << ") " 
    ...
    if (spritesIt != document.MemberEnd())
    {
        for (const auto& currentSprite : spritesIt->value.GetArray())
        {
            ...
            if (framesIt != currentSprite.MemberEnd())
            {
                ...
                for (const auto& currentFrameDesc : framesArray)
                {
                    const std::string subTextureStr = currentFrameDesc["subTexture"].GetString();
                    const uint64_t duration = currentFrameDesc["duration"].GetUint64();
                    const auto pTextureAtlas = getTexture(textureAtlas);
                    const auto subTexture = pTextureAtlas->getSubTexture(subTextureStr);
